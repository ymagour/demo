# チーム開発のデモ
* チーム開発では、codespacesとLive Shareというサービスを用いてモブプロを実施します。
* 以下では、そのcodespacesとLiveShareの実行手順が記載されています
* 最下部に、デモ用の課題がありますので、codespacesとLiveShareを使って挑戦してみてください。
...
## モブプロ実践
* モブプロの練習として、LiveShareを使って、リーダ（ホスト）の環境にあるファイルを参照、更新できることを体験してみましょう。

### LiveShareを使って以下の課題を実施してください
* リーダ（ホスト）の人は画面左のエクスプローラーに練習.txtがあることを確認
* リーダ（ホスト）は、上記で説明した手順に従ってLiveShareのURLを共有（Slackで）してください。
  * <b>以下の作業は、リーダー（ホスト）の人が常にLiveShareの共有を使って実施</b>
* URLを共有してゲストの人がLiveShareに参加できたら、モブプロの役職を決めましょう。
  * ナビゲーター: 1人
  * ドライバー: 1人
  * サポーター、記録係: 上記以外の人
* 役職をきめたら、練習.txtを以下の流れで更新してください
  * ナビゲーターがドライバーに指示を出します。
    * ナビゲーターの好きなもの（食べ物、趣味、アニメ、ゲームなど）を伝える
    * ドライバーはその指示を聞いて練習.txtに入力する
* 入力が終わったら、リーダー（ホスト）はナビゲーターの名前をコメントに入力しコミットします
  * <b>コミットは常はリーダー（ホスト）の人が実施します</b>
* コミットが完了したら、役職を変更してください。これを全員がナビゲーターとドライバーを経験するまで繰り返し行ってください。
